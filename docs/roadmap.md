...
- **Media acquisition — papers + manga BUILT (undeployed), comics deferred.** Built 2026-06-23, committed but **NOT deployed** (operator-gated `just rebuild` on aurora): homelab `87464c8` (suwayomi manga module) · `65cc57b` (paperless + papers-fetch) · `423d660` (aurora wiring). flake-check + aurora-toplevel green.
  ...
  - **Manga**: Suwayomi-Server on aurora, **co-located with komga** (writes `library/manga` directly → komga serves it with zero replication, sidestepping the unbuilt P15 bridge). Self-contained (Tachiyomi/Mihon extensions, no prowlarr/qbittorrent).
  - **Comics (Mylar3) — DEFERRED until P15** (the workstation→aurora `nori.replicas` bridge). Two blockers: Mylar3 needs workstation's prowlarr/qbittorrent but komga is aurora-side (the unbuilt cross-host bridge), AND it'd be the repo's first OCI-container service (vs the native-only bias). Revisit both when P15 lands. **Trigger:** P15 cross-host replicator built.
  - **Books → Readarr** remains an easy \*arr drop-in (unstarted), same cross-host caveat as comics.
...
- **Per-job dirs on the `onetouch` SFTP target are an operator step.** Workstation's `onetouch` repository is `sftp:restic@aurora:` — the bare chroot root. sshd requires `ChrootDirectory` (`/mnt/backup`) to be root-owned and not group/other-writable, so the `restic` user cannot `MkdirAll /<job>`; `initialize = true` fails and a newly declared `nori.backups.<job>` goes red nightly until someone runs `install -d -o restic -g restic /mnt/backup/<job>` on aurora. Declaring the job and provisioning its directory are two copies of the same fact, kept in sync by hand — the bottom rung of the derivation ladder. **Correct shape:** scope the repository under a restic-owned prefix the way pi already does (`…:/pi`, `modules/profiles/entry-plane.nix`), e.g. `…:/workstation`, plus a tmpfiles rule on aurora that creates it; then `MkdirAll` lands inside a restic-owned dir and new jobs self-provision. **Cost:** a one-time relocation of every existing repo on the drive (`mv /mnt/backup/<job> /mnt/backup/workstation/`) — skip it and every repo silently re-initializes empty, losing all history. **Trigger to revisit:** the next job that needs its own repo on `onetouch`, or the next OneTouch re-onboarding (the relocation is nearly free while the repos are already being touched). Bit 2026-08-11 — `docs/reports/20260813-030204-restic-backups-herdr-projects-mcp-onetouch-failure.md`.
