...
- The verified NixOS image remains an offline rollback artifact; it is not an
  active build or deployment path after the appliance moved to Ansible.
...
- [x] Retire the old NixOS Pi deployment path. The inventory now makes Ansible
  the sole deployment owner and preserves the verified image only as an
  offline recovery artifact (2026-09-05).
...
The physical reboot, restore drill, and off-LAN checks remain service-acceptance
gates. They do not restore the retired Nix deployment path; recovery uses the
already verified offline image artifact if the Ansible appliance must be
replaced wholesale.
