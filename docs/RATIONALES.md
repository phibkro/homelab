---
summary: Load-bearing design decisions — why this lab is shaped the way it is.
  Lifted from the dissolved DESIGN.md (2026-06-03) so the "why" doesn't hide
  inside an architecture reference. Each row: decision, rejected alternative,
  evidence it's still right. Newer hard-to-revisit decisions go to `decisions/`
  as ADRs; this is the legacy list.
---

# Rationales

Hard-to-revisit choices, dated. When implementation drifts, fix here or in code — not both. Younger decisions live in `docs/decisions/` as ADRs; this file is the durable list of decisions made *before* the ADR practice was adopted.
...
| **Subvolumes split by value tier, not directory hierarchy** | Subvolumes by topic | Subvolumes are the unit of snapshot/backup policy. Same policy → same subvolume; different policy → different subvolume. | Subvolume table in STORAGE.md and `modules/effects/fs.nix` |
...
| **Pi as appliance + opportunistic backup target** | Pi as "small server" | Marginal cost of a USB HDD on Pi is low; value (fast local restore + observer survives station outage) is high. Failure modes remain independent of workstation. | Pi role in `nori.hosts.pi.role = "appliance"`; restic backup target `/mnt/backup` |
...
| **Blocky over AdGuard Home** | AdGuard Home | Declarative YAML config maps cleanly to `services.blocky.settings`; no web-UI state to drift from declared config. | `modules/server/blocky.nix` |
| **restic over btrbk send/receive for transport** | btrfs-native send/receive | Filesystem-agnostic (Pi backup drive doesn't need to be btrfs); same tool for Pi + Hetzner targets — single mental model. | `services.restic.backups.*` (transport); `services.btrbk.*` (local snapshot only) |
...
| **Backup-correctness via three documented patterns A/B/C** | Trust restic snapshots of running services | Filesystem snapshot of a live database is roulette. Logical dump before backup is the discipline; the patterns document which kind of dump for which kind of service. | Pattern table in SERVICES.md § Backup; live impls in `modules/server/backup/restic.nix` |
...
| **No high availability** | Active-passive failover | Single-PSU residential lab; HA isn't a goal. RTO ladder (RECOVERY.md) makes the tradeoff explicit. | `docs/RECOVERY.md` RTO table; no HA mechanisms |
| **No public internet hosting from the homelab** | Tailscale Funnel / cloudflared on workstation | Personal apps that need public surface live at the Cloudflare edge (Pages + Workers). If a future service ever needs to land public traffic on workstation, Tailscale Funnel is the prototyped path. | `nori.lanRoutes` all `audience != "public-internet"`; Funnel reference preserved in `memory/reference/tailscale_funnel_implementation.md` |
...
Newer hard-to-revisit decisions become ADRs in `docs/decisions/NNNN-*.md` instead of new rows here; this file is the legacy list. The split is by mechanism (when the decision was recorded), not by importance.
