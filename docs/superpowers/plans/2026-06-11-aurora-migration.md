...
| **P12** *(prep done 2026-06-12; final flip pending)* | Cutover: `runsOn` flips to `"aurora"` for each migrated family-tier route (all 10 routes ✓ at P11); Tailscale DNS push order swaps to **pi** primary; `nori.lanIp` flips to pi's LAN IP; Authelia + Caddy disable on workstation. **Prep landed 2026-06-12 (`c015a1a`):** every workstation-resident route opted into `exposeOnTailnet = true` (sonarr/radarr/lidarr/bazarr/prowlarr/jellyseerr/qbittorrent/jellyfin/ollama/open-webui/syncthing/stremio/gatus); syncthing's CLI flag fixed (`guiAddress = "0.0.0.0:8384"` overrides 127.0.0.1 default that the XML setting couldn't reach). Pi's Caddy can now reverse-proxy every workstation backend across the tailnet — verified via curl per port. **Still pending:** lanIp override, hermes Host-header rewrite (DNS-rebinding defense), authelia runsOn flip, WoL prerequisite (P19 — workstation can't sleep until WoL works). | All `https://*.<domain>` reachable through pi's Caddy; workstation Caddy + Authelia disabled. |
