...
| Phase | What | Validation gate |
|---|---|---|
| **P9** ✓ landed 2026-06-11 | Personal residue under `/Users/piplu` (24 GB Pictures/Takeout, CV, work folders, …) extracted by operator; MP510 wiped + reformatted as a single btrfs `mp510-backup` filesystem with 6 subvols (`@backup-local`, `@family-replica-{photos,home-videos,projects,library,archive}`). `@backup-local` mounted at `/mnt/mp510-backup-local` temporarily until P14 migrates the IronWolf `@restic-local` data over and swaps the mount. | MP510 mounted clean at all 6 final paths; 894 GB usable, 6 MB used; `nix flake check` green ✓ |
| **P10** *(in flight)* | Initial sync: workstation `@photos`/`@home-videos`/`@projects`/`@archive` → aurora `/mnt/family/*` via rsync (one-shot, ~464 GB over LAN; `@library` skipped — 480 KiB metadata only). `media-irreplaceable` restic snapshot triggered fresh before kickoff (rollback path). Per-subvol sequential to avoid disk-contention pile-ups. Background script at `/tmp/p10-rsync.sh`, log at `/tmp/p10-rsync.log`. | sha256 manifest match on both sides; the pre-rsync restic snapshot recoverable. |
| **P11** *(in flight)* | Per-service state migration. Same-shape "stop → dump → SFTP → restore → swap" cycle proven 9 times this session: vaultwarden (sqlite bellwether), glance + heim + radicale (stateless), miniflux (Postgres, surfaced the `--no-owner` ownership trap → `[[postgres-ownership-after-dump-restore]]` memory + `/restore-pg-with-owner-fix` skill), filmder + grafana (stateless), calibre-web + komga (no data dependency — library was empty on workstation too). **Remaining:** immich (runbook at `docs/runbooks/immich-cutover.md`; blocked on P10 photos); navidrome (deferred — path mismatch needs operator decision on music tier). | Service-by-service smoke test (UI reachable, data visible, sample operations work). 9 of 11 already verified end-to-end via `https://<sub>.home.phibkro.org` round-trip through workstation Caddy → aurora tailnet IP. |
| **P12** *(reworked per ADR-0003)* | Cutover: `runsOn` flips to `"aurora"` for each migrated family-tier route (the route lift + service-placement landed in P1/P1b/P2/P3 make this a config edit, not a service move); Tailscale DNS push order swaps to **pi** primary; Authelia URLs re-issue OIDC tokens; family clients re-login. Samba shares pre-positioned on aurora 2026-06-11 (samba.nix made host-aware; per-fs `samba = { }` blocks on `/mnt/family/{photos,home-videos,projects,library,archive}`; 445 opened on aurora's tailnet firewall) so family bookmark migration is a one-line URL swap during the cutover window. | All `https://*.<domain>` reachable from aurora; workstation Caddy + Authelia disabled. |
