...
| **P8** | Aurora gains family-tier service declarations with `enable = true` but pointing at empty `/var/lib/<svc>` and `/mnt/family/<X>` (services start, DBs initialize empty). Each enable flips that route's `runsOn` to `"aurora"` so pi's Caddy aims there. | Services up on aurora; reachable via aurora's tailnet IP on internal ports AND via `https://<X>.nori.lan` through pi's Caddy; no data |
...
| **P12** *(reworked per ADR-0003)* | Cutover: `runsOn` flips to `"aurora"` for each migrated family-tier route (the route lift + service-placement landed in P1/P1b/P2/P3 make this a config edit, not a service move); Tailscale DNS push order swaps to **pi** primary; Authelia URLs re-issue OIDC tokens; family clients re-login | All `https://*.nori.lan` reachable; workstation Caddy + Authelia disabled |
