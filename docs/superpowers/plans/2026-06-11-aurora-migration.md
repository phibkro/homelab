...
| Phase | What | Validation gate |
|---|---|---|
| **P15** *(both modules landed 2026-06-12; awaiting key bootstrap)* | btrfs send/receive timer aurora `/mnt/family/*` → workstation MP510 `/mnt/family-replica/*`. Sender (`btrbk-replication.nix`, `04369a6`) + receiver (`btrbk-replica-target.nix`, `ba38187`) both committed and double-gated (hostname + per-service enabled flag). Operator-driven activation: generate the aurora→workstation ed25519 keypair, paste private in sops + pubkey in workstation's btrbk user, flip both `enabled` flags. | First receive completes; subsequent runs are incremental; replication-consistency-verifier (P5) returns clean |
...
| Phase | What | Validation gate |
|---|---|---|
| **P18** *(blocked on diagnosis)* | hypridle suspend re-enabled after the s2idle resume hang is fixed. The 2026-06-10 incident (`e12d34d`): resume from `systemctl suspend` came back to a default background with no keyboard/mouse input — lockscreen never reached an interactive state. Likely libinput/seat reattachment failure on s2idle resume. Hyprlock DPMS fix landed 2026-06-12 (`50a2e18`) but that's a separate listener-ladder bug, not the resume hang. | Resume from suspend works; no lockscreen lockout |
| **P19** *(NIC config landed 2026-06-12 `54a39d8`; magic-packet test blocked on P18)* | Wake-on-LAN tested: pi or aurora sends magic packet, workstation wakes; Caddy/Samba/Jellyfin reachable post-wake. **Landed:** `Wake-on: g` (MagicPacket) on workstation's RTL8125 (enp42s0, MAC 2c:f0:5d:5d:f7:60), via a boot oneshot in `hardware.nix` (scripted-networking doesn't process the .link file the `networking.interfaces.<n>.wakeOnLan` option emits — saved as `[[scripted-networking-link-files-inert]]` memory). End-to-end test (suspend → magic packet → wake) gated on P18 — can't suspend until resume works. | <30s wake-to-service window |
| **P20** *(blocked on P18 + P19)* | Workstation default sleep policy: idle suspend after configurable window; WoL from pi/aurora handles on-demand wake. Power-draw measurement vs the projected baseline. | Observed power draw matches the calculated baseline |
