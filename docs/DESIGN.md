...
**Channel:** `nixos-26.05` stable, pinned via flake.lock (since 2026-06-03; flake commit `4c8eab3`). Re-pin to a newer stable deliberately when the next release ships (~November 2026 for 26.11). The 2026-04-29 attempt to switch to stable 25.11 had failed because services' state had been forward-migrated by unstable (ntfy DB schema v15 vs 25.11's v13, Postgres VectorChord 1.1.1 vs 25.11's 0.5.3, open-webui home-dir regression). The 2026-06-03 retry to 26.05 succeeded with **one** state-strand: Redis dropped from 9.x (unstable) → 8.6.3 (26.05) and `redis-immich`'s RDB format v14 wasn't readable by 8.6.3 — recovery was `mv dump.rdb dump.rdb.vNEW-backup && systemctl start redis-immich` since immich's Redis is cache-class, then `restart immich-server`. Codified as "Downgrading nixpkgs can strand persistent state" in `docs/gotchas.md`. **Pre-move-off-stable rationale (was: Blackwell needed NVIDIA 575+, stable 25.11 lagged) no longer applies** — stable 26.05 ships nvidia 595+ via `nvidiaPackages.production`.
