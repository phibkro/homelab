> Historical proposal; do not execute as a current runbook. Production Pi now uses the Ansible ntfy role with deny-all authentication and SecretSpec inputs.
...
### 2. Edit `nix/modules/system/observability/ntfy/server.nix`
...
**Gatus** (`nix/modules/system/observability/gatus/`) — add the env-file consumption and the `Authorization` header to the ntfy provider config:
...
**`notify@` template** (`nix/modules/system/observability/ntfy/notify.nix`) — add the bearer header to its curl call:
