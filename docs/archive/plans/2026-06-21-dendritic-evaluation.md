...
related: docs/archive/plans/2026-06-21-improve-audit.md § finding #9 + § Direction D-C
---
...
>   docs-replicas) + shared helpers in nix/lib/nixdoc.nix
...
> nix/flake-parts/{formatter,devshell,machines,home,packages/docs-*}.nix
...
1. The "no literal paths" + auto-import philosophy hides where outputs come from. Your existing CLAUDE.md routes operators + agents via explicit paths (`nix/modules/services/<X>.nix`, `docs/reference/<topic>.md`); auto-import inverts that.
2. Your `nix/` NixOS tree is already well-organized + read by both NixOS module system AND tooling (lint walks `nix/modules/services/*.nix` by glob). Auto-import via haumea adds a second indexer; risk of subtle interaction with your existing scanners (`routing-coherence`, `every-service-has-*`).
...
machinesModule wrapper                   1 line   → nix/flake-parts/machines.nix
homeModule wrapper                       1 line   → nix/flake-parts/home.nix
devShells.${system}.default             10 lines  → nix/flake-parts/devshell.nix (perSystem)
packages.${system}                     414 lines  → nix/flake-parts/packages/{
  mkNixdocSection + mkFileDocstring     ~32 lines    → nix/lib/nixdoc.nix (shared)
  docs-lan-route                        ~85 lines    → nix/flake-parts/packages/docs-lan-route.nix
  docs-topology                        ~135 lines    → nix/flake-parts/packages/docs-topology.nix
  docs-capabilities                     ~75 lines    → nix/flake-parts/packages/docs-capabilities.nix
                                                   }
formatter.${system}                      1 line   → nix/flake-parts/formatter.nix
checks.${system}                       358 lines  → nix/flake-parts/checks/{
  statix                                 ~6 lines    → nix/flake-parts/checks/statix.nix
  deadnix                                ~5 lines    → nix/flake-parts/checks/deadnix.nix
  format                                 ~5 lines    → nix/flake-parts/checks/format.nix
  lint (TOML registry dispatcher)       ~12 lines    → nix/flake-parts/checks/lint.nix
  routing-coherence                     ~18 lines    → nix/flake-parts/checks/routing-coherence.nix
  every-service-has-backup-intent       ~40 lines    → nix/flake-parts/checks/services-have-backup.nix
  every-service-has-fs-hardening        ~55 lines    → nix/flake-parts/checks/services-have-hardening.nix
  e2e-pi-smoke                           ~1 line     → nix/flake-parts/checks/e2e-pi-smoke.nix
  e2e-multi-host                         ~1 line     → nix/flake-parts/checks/e2e-multi-host.nix
  e2e-restic-backup                      ~1 line     → nix/flake-parts/checks/e2e-restic-backup.nix
  e2e-disk-alert                         ~1 line     → nix/flake-parts/checks/e2e-disk-alert.nix
  eval-lanroute-customdns              ~10 lines    → nix/flake-parts/checks/eval-lanroute-customdns.nix
  eval-lanroute-port-validation        ~10 lines    → nix/flake-parts/checks/eval-lanroute-port-validation.nix
  eval-route-invariants                ~10 lines    → nix/flake-parts/checks/eval-route-invariants.nix
  eval-gatus-probes                    ~10 lines    → nix/flake-parts/checks/eval-gatus-probes.nix
  docs-fresh                            ~45 lines    → nix/flake-parts/checks/docs-fresh.nix
                                                   }
```
...
### 2. Second indexer over `nix/`

Your `nori.lint`, `every-service-has-fs-hardening`, `every-service-has-backup-intent`, and `routing-coherence` checks all walk `nix/` with glob + grep. They expect a stable file layout under `nix/modules/services/*.nix` and `nix/modules/system/*/default.nix`.

If you adopt haumea or `import-tree` for the flake-parts side, you'd have TWO indexers walking the source tree: one for Nix outputs (auto-import), one for lint scans (your existing globs). Conflicts: a file added under `nix/flake-parts/checks/` would be picked up by haumea but ignored by your lint glob — fine. A file added under `nix/` would be picked up by both — fine as long as the schemas don't collide.

The risk is low but real, and the value (auto-import for `nix/flake-parts/`) is small compared to explicit imports. flake-parts handles fine without haumea.
...
PHASE 1 — Foundation (CI green at each commit)
  ...
  1b. Pull `mkNixdocSection` + `mkFileDocstring` out to nix/lib/nixdoc.nix
      (the "half-measure" from audit finding #9, but as a stepping stone).

PHASE 2 — Easy migrations (one commit per file, CI green)
  2a. formatter → nix/flake-parts/formatter.nix
  2b. devshell → nix/flake-parts/devshell.nix
  2c. machines + home → nix/flake-parts/{machines,home}.nix

PHASE 3 — Packages (one commit per derivation)
  3a. docs-lan-route → nix/flake-parts/packages/docs-lan-route.nix
  3b. docs-topology → … (same shape)
  3c. docs-capabilities → … (same shape)

PHASE 4 — Checks (the bulk; 13 files; one commit per file)
  4a-l. Each check → nix/flake-parts/checks/<name>.nix
...
| Adds `flake-parts` as a foundational dep (must-know for new contributors) | One paragraph in CLAUDE.md § "Quality gates" + a single example file (`nix/flake-parts/checks/format.nix`) used as the exemplar. The pattern is mature, well-documented at flake.parts, low surprise. |
...
| Per-host channel pivots (D-C original framing) — does this enable workstation on master while mac stays on 26.05 cleanly? | YES, flake-parts makes it slightly cleaner via per-host flake-modules overlaying packages selectively. But you can already do this today via the `import nixpkgs-master { ... }` pattern (just landed in nix/home/claude-code/default.nix). Not a NEW capability, just a tidier home. |
...
Pick FULL dendritic (auto-import) when:
  ...
  - You're starting fresh (no existing nix/ to integrate with).
...
3. *"I'd rather just land audit findings #1-4 in the current structure and see if the bloat actually compounds."* → DEFER + land the half-measure (nix/lib/nixdoc.nix extract) instead.
...
- Land audit findings #1+#2+#3 (the test coverage bundle) AFTER Phase 4 — those become "add three new files under nix/flake-parts/checks/ and the Justfile recipes" instead of "diff into the 1055-line file".
- Land audit finding #4 (systemd-execstart-resolves) into the new structure — natural fit.

If DEFER on flake-parts:
- Extract `nix/lib/nixdoc.nix` as the half-measure (shrinks flake.nix by ~150 lines).
...
4. **Auto-format on commit** — your `nixfmt-tree` formatter would still run on the new nix/flake-parts/ tree; verify treefmt config handles the new directory.
