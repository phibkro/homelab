---
summary: "Bundle plan for /improve audit findings #1+#2+#3 — close the silent-drift gap on `nori.harden` and `nori.fs` by adding the two missing layer-3 runtime introspection recipes, then mechanize the meta-rule (`infra-concerns-have-tests`) so future `nix/modules/system/<X>/` additions can't silently land without their test recipe. Two recipes prove the convention; the check then enforces it. Ordered to give immediate value (#1, #2) before the meta-step (#3)."
status: PLAN — awaiting execution
target_commit: 125a07f
audit_source: docs/archive/plans/2026-06-21-improve-audit.md § findings #1, #2, #3
---
...
The three findings share a single insight: **every Reader+Writer-shaped concern in `nix/modules/system/` needs a runtime introspection recipe**, and the convention should be a CI gate not prose. Writing the two missing recipes first (#1, #2) proves the pattern; the meta-check (#3) then ratchets it.
...
Without a CI gate, every new `nix/modules/system/<X>/default.nix` is a silent commitment to ship a `test-<X>` recipe — but the commitment is enforced only by reviewer attention. The two recipes from phases 1 + 2 are the first time the convention is fully realized; this phase ratchets it so the convention can't backslide.

Pattern precedent: `every-service-has-fs-hardening` + `every-service-has-backup-intent` are exemplars. Both walk `nix/modules/services/*.nix` with grep + a case-pattern exemption list and fail the build on uncovered modules.
...
infra-concerns-have-tests = pkgs.runCommandLocal "infra-concerns-have-tests" {
  ...
  for f in $(find nix/infra -maxdepth 2 -name 'default.nix' | sort); do
    if ! grep -qE 'options\.nori\.' "$f"; then continue; fi  # not Reader-shaped

    # Concern name = parent directory name (e.g. nix/modules/system/backup → "backup")
    concern=$(basename "$(dirname "$f")")

    # Expect a `test-<concern>` recipe in the Justfile.
    if ! grep -qE "^@?test-${concern}:" Justfile; then
      echo "✗ nix/modules/system/${concern}/ declares options.nori.* but Justfile"
...
- [ ] Manually remove the `test-harden:` line from `Justfile`, run `nix flake check`, observe RED with `✗ nix/modules/system/capabilities/ declares options.nori.* but Justfile has no test-capabilities recipe.` — wait, the concern is `capabilities` not `harden`. Document this in the check: the test name = directory name, NOT the registry name. (i.e. `nix/modules/system/capabilities/` ⇒ `test-capabilities`, not `test-harden`. Decide.)
...
Pros of directory-name: matches the auto-discovery the check does (`nix/modules/system/<dirname>`).
...
- `nix/modules/services/<X>.nix` test coverage — that's a different concern (see `infra-concerns-have-tests` direction item D-B in the audit).
...
- If Phase 3 reveals an infra concern that legitimately doesn't fit Reader+Writer (e.g. `nix/modules/system/motd.nix` is just a single string), add it to a documented exemption list in the check, mirroring the `baseNonServicePatterns` pattern at the top of `flake.nix § checks.${system}`.
...
Future infra concerns will inherit this convention automatically — adding a new `nix/modules/system/<X>/` with a Reader schema = adding `just test-<X>` (otherwise CI fails). That's the goal.
