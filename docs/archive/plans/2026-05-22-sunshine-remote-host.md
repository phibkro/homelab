...
**Architecture:** A workstation-only NixOS module (`nix/desktop/sunshine.nix`) enables `services.sunshine` with an NVENC-capable package override and KMS capture, scopes its ports to `tailscale0`, and is wired in via `nix/desktop/default.nix`. The MacBook gets Moonlight as a documented Homebrew cask. Pairing and capture are verified at runtime.
...
**Spec deviation (intentional):** the spec mentioned `nori.backups.sunshine.skip`. Dropped — the `every-service-has-{backup-intent,fs-hardening}` flake checks scan `nix/modules/services/*.nix` only; Sunshine is a `nix/desktop/` user service with no homelab state, matching `gaming.nix`/`virt.nix` which carry neither declaration.
...
**Files:**
- Create: `nix/desktop/sunshine.nix`
...
{ pkgs, ... }:
{
  ...
  # nix/desktop/default.nix, which the darwin MacBook never imports
...
Run: `cd /srv/share/projects/homelab && nix-shell -p nix --run 'nix-instantiate --parse nix/desktop/sunshine.nix >/dev/null && echo PARSE OK'`
Expected: `PARSE OK`
...
git add nix/desktop/sunshine.nix
...
**Files:**
- Modify: `nix/desktop/default.nix`
...
Run: `cd /srv/share/projects/homelab && nix-shell -p nix --run 'nix-instantiate --parse nix/desktop/default.nix >/dev/null && echo PARSE OK'`
Expected: `PARSE OK`
...
```bash
cd /srv/share/projects/homelab
git add nix/desktop/default.nix
...
Expected: no assertion failures. In particular, **no** `every-service-has-fs-hardening` / `every-service-has-backup-intent` error naming `sunshine` (it lives in `nix/desktop/`, out of those checks' scope). If one *does* fire, stop and reassess scope — do not silence it.
...
1. Set `capSysAdmin = false;` in `nix/desktop/sunshine.nix` (switches Sunshine to wlroots/`wlr` capture — Hyprland is wlroots-based).
