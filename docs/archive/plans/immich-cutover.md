...
`docs/archive/plans/2026-06-11-aurora-migration.md` § P11. Pattern
...
Edit `nix/modules/services/immich/manifest.nix`:
...
Edit `nix/hosts/workstation/default.nix`:
- `immich.enable = false;`

Edit `nix/hosts/aurora/default.nix` — verify `immich.enable = true;` is <!-- path-coherence: skip — retired historical implementation -->
...
- `docs/archive/plans/2026-06-11-aurora-migration.md` § P11
- `[[postgres-ownership-after-dump-restore]]` memory entry (the
  ALTER OWNER trap)
- `nix/modules/services/immich/manifest.nix` and `nix/modules/services/immich/runtime.nix`
