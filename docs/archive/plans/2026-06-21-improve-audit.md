...
- **Recon**: read `CLAUDE.md`, `docs/glossary.md`, `docs/invariants.md`, `docs/roadmap.md`, `flake.nix` (1032 lines), `nix/` structure, recent commit history (`git log -15`).
...
| **3** | `infra-concerns-have-tests` meta-check not mechanized | Test coverage | HIGH | M | LOW | HIGH | Already on `docs/roadmap.md § Promotion register`. Prevents future regressions like #1 + #2: any new `nix/modules/system/<X>/default.nix` with Reader+Writer shape must ship `test-<X>`. |
| **4** | ~~`systemd-execstart-resolves` flake check~~ | Test coverage | — | — | — | — | **REJECTED on follow-up vet 2026-06-21.** Grep finds ZERO literal-path ExecStarts in `nix/` — every one is `${pkgs.foo}/bin/baz` interpolation, which nix eval already validates (build fails if binary missing). Proposed check would have near-zero catch rate. The actual incident class (2026-06-03, bad flags on wrapped binaries) isn't caught by first-token-resolves anyway; that's covered by the `gotcha-systemd-restart-loop-bombs` skill (prose+procedure rung — appropriate level for this failure mode). |
| **5** | `audience` enum is informational, not enforced | Security | MED | S–M | LOW | HIGH | `audience=family` without an `oidc` or `forwardAuth` block currently lands silently. Schema docstring at `nix/modules/system/networking/default.nix:354` itself flags this as a TODO ("future flake checks may assert consistency"). |
...
| **9** | `flake.nix` is 1032 lines | Tech debt | MED | M (extract) → L (dendritic refactor) | LOW | HIGH | **Superseded by operator's dendritic-pattern question.** Half-measure: extract `mkNixdocSection`/`mkFileDocstring` to `nix/lib/docs.nix` (saves ~150 lines). Full-measure: dendritic via `flake-parts` (saves ~900 lines, all checks/packages become per-file). See § "Dendritic decision" below. |
...
|---|---|
| qBittorrent TempPath not in `nori.harden.qbittorrent.binds` | Hypothetical bug — `nix/modules/system/capabilities/default.nix:125` explicitly states `ProtectSystem=strict` is deliberately NOT applied. The "latent if hardening tightens" framing assumes a future config change that's been explicitly rejected. |
...
| filmder/heim `audience=public` lacks documented threat model | Already documented — `nix/modules/services/filmder.nix:52-55` clarifies `public` = tailnet-public, not internet-public. Real gap is the *schema*-level doc (see finding #5). |
...
| Media group declared in 3 places (arr/shared, immich, aurora) | Already acknowledged — `nix/modules/system/capabilities/default.nix:17` comment names this with explicit deferral. Group membership is idempotent; current cost ≤ refactor cost. |
...
| `access` infra concern lacks `options.nori.*` Reader schema | Verified: `nix/modules/system/access/default.nix` is a 24-line import aggregator. Adding a Reader schema for a 24-line module is premature abstraction (violates rule-of-three). |
...
- "It's long but not painful" → land half-measure (extract `nix/lib/docs.nix`), defer dendritic to next inflection point.
...
Pick which findings to turn into individual implementation plans (default suggestion: #1-bundle + #5 + #6 + #4 — the top 4 by leverage). Each selected plan gets its own file at `docs/archive/plans/2026-06-21-<slug>.md` following the project's existing plan-file convention.
