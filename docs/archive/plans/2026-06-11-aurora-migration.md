...
| Service module sweep (every `nix/modules/services/*.nix` wrapped in `mkMerge` + `mkIf cfg.enable`) | Mechanical refactor | P2 |
...
| **P2** | Service module sweep: wrap each `nix/modules/services/*.nix` in the gated-activation shape; assign `tags` per module; default `enable = false`; the wrap is mechanical | `nix-diff` per host = empty (semantic-equivalence) |
...
- `nix/modules/system/backup/default.nix` — `nori.backupTargets` schema (remote-URL support landed 2026-06-10)
- `nix/modules/system/storage/default.nix` — `nori.fs` schema (extension for `samba` block lands in P4)
- `nix/modules/system/networking/default.nix` — `nori.lanRoutes` (extensions for `port` aggregator + `upstreams` land in P1)
