...
- `docs/archive/plans/2026-06-11-aurora-migration.md` — the *how*.
  Phase-by-phase (P1 through P20). This is the source-of-truth that
  every subsequent commit checks against.
- `docs/archive/plans/2026-06-11-docs-shape-review.md` — meta
...
1. **Foundation effect modules** (P1 + P1b + P2 + P3 + P4):
   - `nix/modules/system/placement.nix` (NEW) — `nori.services.<X>.{enable,tags,enabled}` registry + `nori.enableServicesByTag`. Lets hosts opt-in by name or by tag.
   - `nix/modules/system/networking/default.nix` — adds `runsOn` field. Routes now declare cross-host placement; the proxy host (whichever runs Caddy) resolves the upstream via `routeHost` (127.0.0.1 if local, tailnet IP otherwise).
   - `nix/modules/system/storage/default.nix` — adds optional `samba = { … }` block per entry; generator emits Samba shares + ownership tmpfiles. Same fs entry stays the source of truth for path + tier + share.
   - **Every `nix/modules/services/*.nix`** wrapped in `mkMerge` + `mkIf cfg.enabled` (commit `714aa10`). Route declarations lift outside the activation gate so the route registry is visible host-wide even when the service itself is disabled (lets proxy hosts know how to route to the backend running elsewhere). **This was a big mechanical refactor — semantically a no-op per host.**
...
2. **Aurora bootstrap** (P6 + P13):
   ...
   - `nix/modules/services/backup/restic-target.nix` (NEW) — chrooted SFTP user `restic` on aurora so workstation + pi can push restic snapshots over the tailnet. `mkAfter` trick on `openssh.extraConfig` avoids the PrintLastLog trap that wedges the Match block (`c3ba27f`).
...
# expect: every nix/modules/services/*.nix touched, plus nix/modules/system/
...
**Verify:**
```sh
git diff cp/02-foundation-and-bootstrap..cp/03-entry-plane-le -- nix/modules/system/networking/caddy.nix
...
**Verify:**
```sh
git diff cp/07-mp510-p9..cp/08-cutovers-batch-B-and-p14 -- nix/modules/services/backup/restic.nix
# target rename
git log cp/07-mp510-p9..cp/08-cutovers-batch-B-and-p14 -- nix/modules/services/calibre-web.nix nix/modules/services/komga.nix
# cutover commits
```
...
```sh
# read this doc
cat docs/archive/plans/2026-06-11-pr-review-stack.md
...
- P11 immich cutover — runbook ready (`docs/archive/plans/immich-cutover.md`), data move blocked on P10.
