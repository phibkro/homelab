...
`nix/modules/system/resource-tiers.nix` declares per-service `MemoryHigh`/`MemoryMax` tiers plus
...
`nix/modules/system/<X>/default.nix` (flake.nix ~1088). This is a *flat* file, invisible to that
...
| 2 | **Close the orphan-module lint gap** | Extend the flake check to assert every `nix/infra` file declaring `options.nori.*` is reachable from some host's module list. This is the *test* rung that would have caught defect 1 — currently convention-only. |
