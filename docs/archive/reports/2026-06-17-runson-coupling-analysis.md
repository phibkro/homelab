...
```
machines/aurora/default.nix imports nix/modules/services/vaultwarden.nix
...
```
$ grep -rn "runsOn\s*=" nix/ machines/ --include="*.nix" | wc -l
34
```
...
Today:
- `nori.lanRoutes` lives in `nix/modules/system/networking/default.nix` (single file, ~700 lines)
- `nori.backups` lives in `nix/modules/services/backup/` (a folder — backup.nix + restic.nix + verify.nix + btrbk.nix)
- `nori.harden` lives in `nix/modules/system/capabilities/default.nix` (single file)
- `nori.fs` lives in `nix/modules/system/storage/default.nix` (single file)
