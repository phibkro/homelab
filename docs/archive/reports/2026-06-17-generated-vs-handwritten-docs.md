---
date: 2026-06-17
summary: Side-by-side comparison of generated vs handwritten docs after the option-E experiment landed (file-level /** */ extraction + narrative migration into nix/modules/system/networking/default.nix + nix/lib/machines.nix). Verdict — generated docs CAN carry module-scoped narrative; cross-module synthesis still needs handwritten authoring.
---
...
|---|---|
| **Authelia OIDC overview** | Authelia is access-concern (`nix/modules/system/access/`), not networking. A full overview belongs in that module's docstring, not networking's |
| **Tailscale** | Separate module (`nix/modules/system/networking/tailnet-appliance.nix`); fragmenting the overview across two extraction sites loses cohesion |
...
| Section | Why it can't migrate |
|---|---|
| **Pi posture** (anti-write storage, NVMe enumeration warning) | Lives in `nix/hosts/pi/hardware.nix`. Could be extracted by adding pi/hardware.nix as a third docs-topology section, but conceptually it's a per-host concern, not a topology-as-a-whole concern |
...
| **GPU access pattern** | Lives in `nix/modules/system/capabilities/gpu.nix`. Belongs in a docs-capabilities generator (not built yet) |
...
3. (optional) add docs-capabilities generator extracting from
   nix/modules/system/capabilities/{gpu,default}.nix — pulls GPU access
