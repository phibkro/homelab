...
summary: Post-execution report for the aurora migration arc. Companion
  to the forward plan in `docs/archive/plans/2026-06-11-aurora-migration.md`
...
> plan (`docs/archive/plans/2026-06-11-aurora-migration.md`) and
...
| Files | `home/desktop/hypr-lock.nix` (CADDY_AUTO_TRUST note), `nix/modules/services/navidrome.nix`, `nix/modules/services/backup/btrbk-replication.nix` |
...
| What | Path renames (`nix/server` → `nix/services`), drive-name corrections (`ironwolf` → `mp510`), URL updates (`*.nori.lan` → `*.home.phibkro.org`). ROADMAP gains a "do a full deep-scan post-migration" TODO. |
...
|---|---|
| Commit | `8800d5f` |
| Files | `nix/modules/services/immich.nix`, `machines/workstation/default.nix` |
...
| Files | 13 modules under `nix/modules/services/{arr/,}*.nix`, `nix/modules/services/syncthing.nix` (guiAddress fix), `nix/modules/services/backup/btrbk-replica-target.nix` (new), `machines/workstation/hardware.nix` (WoL) |
...
Two gaps: `nix/modules/system/observability/beszel/agent.nix` was never imported on
...
|---|---|
| Commit | `fccf922` |
| Files | `home/hermes/default.nix`, `nix/modules/services/hermes.nix` |
...
|---|---|
| Commit | `0629326` |
| Files | `nix/common/default.nix`, `nix/modules/system/access/authelia.nix`, `nix/modules/services/default.nix` (hermes added to bundle), `machines/workstation/default.nix`, `machines/pi/default.nix` (gatus.exposeViaCaddy override dropped) |

The architectural flip itself, in one commit:

- `nori.lanIp = config.nori.hosts.pi.lanIp` in `nix/common/default.nix`
...
| Files | `docs/archive/plans/2026-06-11-aurora-migration.md`, `docs/roadmap.md`, `docs/decisions/0003-pi-central-entry-plane.md`, `docs/decisions/0004-letsencrypt-on-home-phibkro-org.md`, `nix/modules/services/jellyfin.nix` |
...
|---|---|
| Commit | `2877267` |
| Files | `nix/modules/services/backup/btrbk-replication.nix`, `nix/modules/services/backup/btrbk-replica-target.nix`, `machines/aurora/default.nix`, `machines/workstation/default.nix`, `secrets/secrets.yaml` |
...
|---|---|
| Commit | `3ae2576` |
| Files | `docs/archive/plans/2026-06-11-aurora-migration.md` |
...
- `docs/archive/plans/2026-06-11-aurora-migration.md` — the forward plan with per-phase tables, validation gates, reversibility ladder. Tracks the "what" and the "when".
