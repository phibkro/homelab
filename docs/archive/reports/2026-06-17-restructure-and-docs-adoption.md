...
E exp.   0bd2559   extract file-level /** */ docstrings into generated
                    (awk pre-pass added to mkNixdocSection; narrative
                     + diagrams migrated to nix/modules/system/networking/
                     default.nix + nix/lib/machines.nix)
...
phase 1   0b8c384   backup concern → nix/modules/system/backup/
...
phase 3f  9d63fd5   drain nix/effects/ — empty after Phase 3 sweep

phase 4   5e35815   ./home/ + ./machines/ → nix/{home,machines}/
...
phase 5b  63b7225   delete nix/dev — per-project concern, not homelab's
phase 5c  b7e9a84   extract lint to /lint at root + ripgrep
                    (was nix/lint/; tooling, not config)
...
phase 6a  7bb8a82   nix/common/ → nix/modules/system/base/
phase 6b  2bb9de7   nix/desktop/ → nix/modules/system/desktop/
...
✓ ~120 RFC 145 doc-comments seeded across nix/
...
✓ scope-aligned nix/ tree:
    nix/hosts/ = NixOS-system scope
    nix/home/     = home-manager scope
    nix/modules/system/    = PaaS infra (consumed)
    nix/modules/services/ = workloads (consumed)
...
| Value | Status |
|---|---|
| V1. Correctness first | ✓ Phase 5b (nix/dev deletion) embraced per-project framing rather than salvaging the abstraction. Phase 5a chose explicit imports + assertion over readDir magic. |
...
✓ side-by-side comparison report captures the option-E findings for
  future reference (docs/archive/reports/2026-06-17-generated-vs-handwritten-
  docs.md)
...
```
nix/home/{core,pc}.nix → nix/home/base/
    Q1 bias deferred — nix/home/ is already mono-scope; symmetry
    buys no clarity. Re-open if a third HM tier emerges.

nix/modules/system/ + nix/modules/services/ → nix/hosts/
...
2. nix/home/ restructure if a third tier emerges
   Q1 bias was "wait for rule-of-three". Today nix/home/ has
   core.nix + pc.nix + 3 component folders. If a fourth concern arrives,
   reconsider nix/home/base/ structure.
...
6. extend option-E coverage to remaining handwritten concerns
   ...
   shape (each pairs with its corresponding nix/modules/system/<concern>/
   default.nix file-level docstring). Each is a ~half-day pass; not
   in scope here.

7. consider extending the mkFileDocstring → docs-<X> pattern to
   per-service modules
   Each nix/modules/services/<X>.nix could get a generated artifact at
