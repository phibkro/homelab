...
units get both — `nix/modules/system/backup/default.nix` does
...
`nix/modules/system/backup/restic.nix` emitted `"<job> <target> <repoPath>"` lines
...
`nix/modules/system/backup/restic.nix` — replace the whitespace-delimited heredoc
