...
> `git diff --stat 0cef85b..HEAD -- tests/tests.just nix/modules/system/storage/replication.nix docs/reference/runtime-tests.md`
...
- **Effort**: S
- **Risk**: LOW
- **Depends on**: `docs/archive/legacy-plans/005-place-replica-intent-on-target.md`, `docs/archive/legacy-plans/006-fail-closed-backup-runtime-tests.md` (serializes `tests/tests.just`)
- **Category**: bug, tests
...
- `nix/modules/system/storage/replication.nix:141-163` defines a read-only script: inspect target directory, find newest snapshot mtime, compare with `maxAgeHours`.
- `nix/modules/system/storage/replication.nix:167-176` schedules the same verifier hourly.
...
- `tests/tests.just`
- `nix/modules/system/storage/replication.nix` — comment or unit metadata only if needed
