...
- **Effort**: M
- **Risk**: MED
- **Depends on**: `docs/archive/legacy-plans/009-claim-music-before-ingest.md` (serializes shared `flake.nix` edits)
...
- `nix/home/claude-code/default.nix:193-196` sets `permissions.defaultMode = "auto"` and suppresses the dangerous-mode prompt.
- `nix/home/claude-code/default.nix:344-373` defines `importSkills`, flattening upstream directories into `~/.claude/skills`.
- `nix/home/claude-code/default.nix:393-495` imports selected skills from superpowers, caveman, Matt Pocock, Anthropic, shadcn, shadcn-improve, and Obsidian repositories.
...
- `nix/home/claude-code/third-party-skills.nix` — create
- `nix/home/claude-code/third-party-skill-manifest.json` — create, generated
- `flake.nix`
...
- Changing locally authored skills under `nix/home/claude-code/skills/`.
...
Create `nix/home/claude-code/third-party-skills.nix` as pure data. Each collection entry must include:
...
```bash
nix eval --json --file nix/home/claude-code/third-party-skills.nix
```
...
Generate and commit `nix/home/claude-code/third-party-skill-manifest.json` by copying the package output. This file is an **integrity baseline only**: it means “these exact directories were recorded,” not “their instructions were security-reviewed.” Name the check and documentation accordingly.
...
cmp /tmp/skill-manifest nix/home/claude-code/third-party-skill-manifest.json
```
