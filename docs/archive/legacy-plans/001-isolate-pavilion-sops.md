...
> **Executor instructions**: Follow this plan step by step. Run every verification command and confirm the expected result before moving to the next step. If anything in the **STOP conditions** occurs, stop and report; do not improvise. When done, update this plan's row in `docs/archive/legacy-plans/README.md` unless the reviewer who dispatched you owns the index.
>
> **Drift check (run first)**:
> `git diff --stat 0cef85b..HEAD -- .sops.yaml secrets/README.md secrets/secrets.yaml secrets/apps.yaml secrets/beszel-agent.yaml secrets/pavilion.yaml nix/modules/system/observability/beszel/agent.nix lint/checks/sops-recipient-boundaries.sh flake.nix`
>
...
- `nix/modules/system/base/sops.nix:23-40` configures each host to decrypt with `/etc/ssh/ssh_host_ed25519_key` and defaults all declarations to `secrets/secrets.yaml`.
- `nix/hosts/pavilion/default.nix:17-20` explicitly persists pavilion's SSH host keys across reboots.
- `nix/modules/system/observability/beszel/agent.nix:66-83` declares only `beszel-hub-pubkey` through SOPS. That value is a public trust key, but all hosts currently obtain it from the privileged default file.
...
```nix
# nix/modules/system/observability/beszel/agent.nix:72-83
...
- `nix/modules/system/observability/beszel/agent.nix`
...
Change `nix/modules/system/observability/beszel/agent.nix` to accept `inputs` and set:
...
- [ ] `docs/archive/legacy-plans/README.md` status row is updated.
