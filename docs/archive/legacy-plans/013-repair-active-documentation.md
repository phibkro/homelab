...
> `git diff --stat 0cef85b..HEAD -- README.md docs/README.md docs/reference/module-authoring.md docs/reference/runtime-tests.md nix/hosts/aurora/default.nix nix/modules/services/immich.nix nix/modules/services/miniflux.nix lint/checks/routing-coherence.sh docs/invariants.md`
...
- **Effort**: M
- **Risk**: LOW
- **Depends on**: `docs/archive/legacy-plans/007-resolve-manual-backup-units.md`, `docs/archive/legacy-plans/012-verify-authelia-client-registry.md` (serializes `README.md` and runtime-test docs)
- **Category**: docs, dx
...
- Root `README.md:86-120` still shows `nix/effects/` and top-level `home/`, while current code is `nix/modules/system/` and `nix/home/`.
...
- `nix/hosts/aurora/default.nix:9-31` says Aurora is a single-role Immich ML offload and authoritative state remains on workstation; lines 75-82 say family services are still standing up empty. The same file enables the full family-vault workload at lines 91-113.
- `nix/modules/services/immich.nix:27-38` and `nix/modules/services/miniflux.nix:46-47` give active setup instructions with `*.nori.lan`; canonical access is `*.home.phibkro.org` through `config.nori.domain`.
...
| Search active drift | `rg 'GLOSSARY\.md|TOPOLOGY\.md|nix/effects|server/|\.nori\.lan' README.md docs/README.md docs/reference nix/hosts/aurora nix/modules/services/{immich,miniflux}.nix` | no unintended active references |
...
- `nix/hosts/aurora/default.nix` — comments only
- `nix/modules/services/immich.nix` — comments/instructions only
- `nix/modules/services/miniflux.nix` — comments/instructions only
- `lint/checks/routing-coherence.sh`
- `docs/invariants.md` — routing-check coverage description only

**Out of scope**:

- Historical files under `docs/decisions/`, `docs/archive/plans/`, `docs/archive/reports/`, or dated `docs/specs/` unless an active entry point links to a nonexistent path.
...
- `nix/modules/system/` for platform concerns;
- `nix/home/` for home-manager modules;
- folders under `nix/modules/services/` signal coupling;
...
rg 'nix/effects|server/' README.md docs/reference/module-authoring.md docs/reference/runtime-tests.md
```
...
- `nix/modules/system/capabilities/default.nix` for `nori.harden`;
- `nix/modules/system/storage/default.nix` for `nori.fs`;
- `nix/modules/system/motd.nix` rather than `rust-motd.nix`;
- `nix/modules/system/`, not `effects/`;
...
rg '\.nori\.lan|standing up empty|single-role' \
  ...
  nix/modules/services/immich.nix \
  nix/modules/services/miniflux.nix
```
...
- [ ] Active architecture docs use `nix/infra`, `nix/home`, and `nix/services` correctly.
...
- [ ] `docs/archive/legacy-plans/README.md` status is updated.
