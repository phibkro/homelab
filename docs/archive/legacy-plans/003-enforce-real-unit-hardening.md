...
> `git diff --stat 0cef85b..HEAD -- nix/modules/system/capabilities/default.nix nix/modules/system/backup/btrbk-replication.nix nix/modules/system/backup/btrbk-replica-target.nix nix/modules/system/backup/restic-target.nix tests/tests.just tests/eval/harden-invariants.nix flake.nix docs/invariants.md`
...
- **Effort**: M
- **Risk**: MED
- **Depends on**: `docs/archive/legacy-plans/002-rotate-console-credentials.md` (serializes shared `flake.nix` edits)
...
- `nix/modules/system/capabilities/default.nix:43-45` says each attribute name **must** match a systemd unit.
- The writer at `nix/modules/system/capabilities/default.nix:107-156` creates `systemd.services.<name>.serviceConfig`, which also makes typo names appear in the module tree.
- The baseline includes `TemporaryFileSystem = [ "/mnt:ro" "/srv:ro" ]`, `ProtectHome`, `NoNewPrivileges`, and related properties.
- `nix/modules/system/backup/btrbk-replication.nix:81-103` creates `btrbk-family-replica`, but line 173 declares `nori.harden.btrbk-replication = { };`.
...
- `nix/modules/system/backup/restic-target.nix:83-89` explicitly says no `restic-target` unit exists, then declares a profile with that name.
- `nix/modules/system/backup/btrbk-replica-target.nix:21-35` is authorization-only but also declares a non-unit profile.
...
- `nix/modules/system/capabilities/default.nix`
- `nix/modules/system/backup/btrbk-replication.nix`
- `nix/modules/system/backup/btrbk-replica-target.nix`
- `nix/modules/system/backup/restic-target.nix`
- `tests/tests.just`
...
- Adding `ProtectSystem=strict`; it is explicitly deferred at `nix/modules/system/capabilities/default.nix:125-130`.
...
In `nix/modules/system/capabilities/default.nix`, add assertions for every `nori.harden` entry. A profile is valid only when the merged service definition has a real executable body, accepted in one of the NixOS forms used by this repository:
