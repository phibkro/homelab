...
> **Executor instructions**: Follow this plan exactly. Never print, paste into logs, or commit a password or password hash. Run every verification gate before proceeding. Stop on any condition listed below rather than improvising. Update this plan's status in `docs/archive/legacy-plans/README.md` when complete unless the dispatcher owns the index.
>
> **Drift check (run first)**:
> `git diff --stat 0cef85b..HEAD -- nix/hosts/aurora/default.nix nix/hosts/pavilion/default.nix secrets/secrets.yaml secrets/pavilion.yaml lint/rules.toml tests/eval/console-password-files.nix flake.nix`
...
- **Effort**: S
- **Risk**: LOW
- **Depends on**: `docs/archive/legacy-plans/001-isolate-pavilion-sops.md`
- **Category**: security
...
- `nix/hosts/pavilion/default.nix:288-295` labels the credential temporary and configures `users.users.nori.hashedPassword` inline.
- `nix/hosts/aurora/default.nix:340-345` uses the same temporary pattern.
- `nix/modules/system/base/users.nix:9-16` places `nori` in `wheel`.
- `nix/modules/system/base/users.nix:35-41` sets `security.sudo.wheelNeedsPassword = false`.
...
Add a `nori.lint` rule in `lint/rules.toml` that rejects inline `users.users.*.hashedPassword =` declarations under `nix/`. The message must direct authors to `hashedPasswordFile` backed by SOPS. Scope the pattern narrowly enough not to reject documentation that names the option.
...
- Static lint: inline `hashedPassword` under `nix/` is forbidden.
...
- [ ] No inline `users.users.*.hashedPassword` remains under `nix/`.
