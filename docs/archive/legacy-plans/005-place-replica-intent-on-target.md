...
> `git diff --stat 0cef85b..HEAD -- nix/modules/system/backup/btrbk-replication.nix nix/modules/system/storage/replication.nix tests/eval/replica-placement.nix flake.nix docs/reference/storage.md docs/generated/replicas.md`
...
- **Effort**: M
- **Risk**: MED
- **Depends on**: `docs/archive/legacy-plans/004-authenticate-suwayomi-api.md` (serializes shared `flake.nix` edits)
- **Category**: bug
...
- `nix/modules/system/backup/btrbk-replication.nix:69-70` gates the whole sender configuration on Aurora.
- `nix/modules/system/backup/btrbk-replication.nix:135-156` declares `nori.replicas` **inside** that gate, deriving entries from Aurora's local `nori.fs`.
- `nix/modules/system/storage/replication.nix:113-123` filters the local registry by `target.host == config.networking.hostName` before emitting units.
...
- Workstation already declares target filesystem entries such as `family-replica-library` in `nix/hosts/workstation/disko-mp510.nix:37-40`.
...
- `nix/modules/system/storage/replication.nix` — assertions or comments only if needed
