...
2. **Split shape**: `nix/modules/services/<service>/{daemon,client}.nix` where the daemon is the actual server (lives on Pi) and the client is the proxy / notification template / agent (lives on every host).
...
4. **Backup decision**: Pi's anti-write posture means `nori.backups.<n>.skip = "..."` for the daemon — host-aware assertion in `nix/modules/system/backup/default.nix` enforces this.
...
```bash
# Before:
nix/modules/services/<service>/{manifest,runtime}.nix

# After:
nix/modules/services/<service>/
...
```nix
# nix/modules/services/<service>/daemon.nix      # path-coherence: skip — template
{ config, lib, pkgs, ... }:
{
  services.<service> = {
    enable = true;
  ...
  nori.backups.<n>.skip = "Hub on appliance host. Pi flash anti-write posture; <reason data is non-load-bearing>. Defer until Pi gains the planned local fast-restore disk repo (see nix/modules/system/backup/restic.nix).";
}
```
...
```nix
# nix/modules/services/<service>/client.nix      # path-coherence: skip — template
{ config, lib, pkgs, ... }:
{
...
── Split-module shape ────────────────────────────────────────────
nix/modules/services/<service>/{daemon,client}.nix.
