...
> `git diff --stat 0cef85b..HEAD -- nix/modules/services/music-ingest.nix nix/modules/services/music-ingest.sh nix/modules/services/music-ingest.test.sh nix/hosts/workstation/default.nix docs/runbooks/music-flac-ingest.md flake.nix`
...
- **Effort**: M
- **Risk**: MED
- **Depends on**: `docs/archive/legacy-plans/006-fail-closed-backup-runtime-tests.md` (serializes shared `flake.nix` edits)
- **Category**: bug
...
- `nix/modules/services/music-ingest.sh:98-108` checks temporary sibling and age before copying.
- `nix/modules/services/music-ingest.sh:135-151` copies, fsyncs, renames into the master, then removes the original staging path without revalidating ownership.
- `nix/modules/services/music-ingest.test.sh` covers stable/fresh files, temp siblings, dedupe, conflicts, permissions, art, and rerun idempotence, but not replacement during ingest or crash recovery.
- Workstation config:
  - staging: `/mnt/media/staging/music-flac` (`nix/hosts/workstation/default.nix:169-180`);
...
|---|---|---|
| Fixture | `nix shell nixpkgs#b3sum --command bash nix/modules/services/music-ingest.test.sh` | all cases pass |
...
- `nix/modules/services/music-ingest.nix`
- `nix/modules/services/music-ingest.sh`
- `nix/modules/services/music-ingest.test.sh`
...
```bash
nix shell nixpkgs#b3sum --command bash nix/modules/services/music-ingest.test.sh
```
