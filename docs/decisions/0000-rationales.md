...
| **Hyprland as the initial desktop, with Plasma as an alternative** | GNOME Shell or two login managers | Hyprland keeps the declarative keyboard-first rice. Plasma provides a conventional session without replacing it. One filtered greetd chooser owns both session entries. | `profiles/desktop/nixos/graphical-desktop.nix`; `services/greetd/nixos.nix`; `docs/specs/2026-09-22-dual-desktop-sessions.md` |
| **`nixos-unstable` as the primary package set, with bounded pins** | Move the full system to stable; consume `master` broadly | The workstation needs current kernel, desktop, and Blackwell GPU support. `flake.lock` still makes upgrades deliberate. A separate 26.05 input serves HandBrake, and `master` supplies only named lagging packages. | Primary, stable, and master inputs and their documented consumers in `flake.nix`; exact revisions in `flake.lock` |
