...
| **Workstation root snapshot history sent to the IronWolf with btrbk** (2026-09-26) | Keep `7d 4w 6m` root history on the system NVMe; keep history only in restic; send the pre-cleanup history on the first run | Root history pinned about 285 GiB of deleted and rewritten data on the 86%-full system NVMe, and same-disk snapshots are lost with that disk. Btrbk send/receive puts read-only copies on the IronWolf btrfs that restore like local snapshots. It adds history, not transport: the IronWolf is inside the same host, so restic to the OneTouch remains the independent backup. Two phases because btrbk backfills every local snapshot its target policy selects: phase 1 keeps only the latest copy (about 326 GiB instead of 561 GiB); `4w 6m` follows in a reviewed commit once the pre-cleanup dailies have aged out. | `retention.workstationRoot` in `src/inventory/backup.nix` (current and planned values, `notBefore`); root target in `src/services/btrbk/nixos.nix`; `eval-btrbk-root-offload` check; phase-2 item in `docs/roadmap.md` |
