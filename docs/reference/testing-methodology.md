...
summary: How testing works in this homelab — the three-layer pyramid
  ...
  TDD viable, and the real-values-not-stubs principle that keeps
  integration tests honest. Companion to `runtime-tests.md` which
  covers the live-system introspection layer in detail.
---
...
tests/                          pre-deploy tests
  ...
  keys/test-age.{txt,pub}        test-only age key (committed in
                                  plaintext on purpose — cannot
                                  decrypt production secrets, see
                                  scripts/regen-test-secrets.sh)
  secrets/test.yaml              sops-encrypted secrets file
                                  encrypted to the test recipient,
                                  consumed by e2e tests via real
                                  sops-nix activation
  .sops.yaml                     test-only sops creation rule

scripts/regen-test-secrets.sh   one-shot: refreshes the test key +
                                 re-encrypts test secrets. Run when
                                 a module declares a new sops.secret.
...
## Real values, not stubs — the integration-test principle

**Stubbing is for unit tests. Integration / e2e tests verify
composition — and the moment you stub a piece of the composition, you
stop testing what you actually want to test.**

This principle came out of the Phase 5 / Phase 6 work on authelia:
the first attempt stubbed sops-nix at the option layer (a
`sops-stub.nix` fixture that planted dummy files at the secret paths)
and authelia failed to start because it actually parses every secret
at startup. The fix was not "better stubs" — it was "stop stubbing."
The Phase 6 design uses the REAL sops-nix module against a real sops-
encrypted YAML file with real-shape values (RSA key generated by
openssl, argon2id hash by argon2, etc).

```
WRONG  stub the module surface; tests pass; production breaks
       because the real module reads CONTENT, not just paths.

RIGHT  use the real module; supply REAL test data — keys + values
       generated to the same shape production uses.
```

In practice for this homelab:

| Concern | Real-values approach |
|---|---|
| sops decryption | Real sops-nix + `tests/keys/test-age.txt` + `tests/secrets/test.yaml` |
| Authelia argon2id user hash | `argon2 ... -id -e` in `scripts/regen-test-secrets.sh` |
| Authelia RSA OIDC issuer key | `openssl genrsa 2048` in regen script |
| TLS for Caddy | Caddy's built-in `local_certs` (internal CA) — IS a real TLS impl |
| Cloudflare ACME | Bypassed via `local_certs` — token still encrypted in test.yaml so the secret manifest decrypts cleanly |
| ntfy publish endpoint | Stubbed for now (not in critical path); Phase 7 adds a real ntfy server in-VM |
| healthchecks.io | Stub URL; heartbeat.service runs but curls fail (the timer activation is what we assert, not the post) |

The few stubs that remain are at clear external boundaries (the wider
internet, paid third-party services). EVERYTHING the homelab itself
declares is exercised by the real module against real values.

### When to write a real fixture vs a stub

```
DEFAULT     real value. The whole point of an integration test is to
            test composition. Stubbing IS lying about composition.

EXCEPTION   the dependency is OUTSIDE the system being tested AND
            cannot run in the test environment. ntfy.sh, hc-ping.io,
            cloudflare's ACME API — these are stubbed because the
            test VM has no internet AND we don't own those services.

PROMOTE     when a "stubbed" dependency is something we DO control
            (or could run in-VM cheaply), promote it: stand up the
            real thing in the test VM and post real messages. That's
            the difference between Phase 6 (authelia: real) and the
            current ntfy stub (eventually → Phase 7).
```

### Generating test secrets

`scripts/regen-test-secrets.sh` is the canonical generator. It produces:

- A test-only age keypair (committed in plaintext — cannot decrypt
  production secrets, which are encrypted to a separate recipient set).
- All the secret values declared anywhere in the homelab modules:
  every `sops.secrets.<name>` that the e2e test exercises must have a
  value in `tests/secrets/test.yaml`, or sops-install-secrets will
  fail the manifest check at VM build time.
- A `tests/.sops.yaml` policy file that scopes the test recipient to
  the `tests/` subtree.

Add a new secret to the script + re-run when adding a new module that
declares a sops secret. CI catches the gap automatically.
...
✗ mock the system under test
  bad: stub sops-nix at the option layer so authelia "starts" without
       real keys. The composition you're stubbing IS what you're
       supposed to test.
  good: real sops-nix + real test secrets + real argon2/RSA values.
        See "Real values, not stubs" above.
