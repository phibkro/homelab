...
guidance — `src/users/nori/programs/agent-soul/SOUL.md`, the generated
...
`src/services/agent-fix/nixos.nix` deliberately calls `pagu-box`, **not**
...
Claude and Codex from one pinned source (`src/users/nori/programs/agent-skills/`), so they
