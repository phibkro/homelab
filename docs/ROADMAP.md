...
- **Aurora migration — workstation-as-compute / aurora-as-family-vault.** Reorganises hosts to let workstation sleep when no GPU/transcode/bulk-storage workload is active and to give irreplaceable media a 3-copy replication posture. Full delta table, phase ordering, validation gates, and reversibility ladder in `docs/superpowers/plans/2026-06-11-aurora-migration.md`. **Progress (2026-06-12):** P1–P11 ✓ landed; P10 (220 GB photos + 48 GB music on aurora), P11 (all 10 family-tier services on aurora — immich + navidrome were the last two). P12 prep ✓ (workstation tailnet ports opened per-route, syncthing UI rebound, WoL enabled). P15 sender + receiver modules drafted (`04369a6` + `ba38187`); awaiting operator key bootstrap to activate. **Outstanding for the actual P12 flip:** `nori.lanIp` to pi, hermes Host-header rewrite, authelia runsOn flip, P19 (workstation can't sleep until the s2idle resume hang is fixed — documented in `e12d34d` commit message).
