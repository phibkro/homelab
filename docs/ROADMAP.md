...
- **Pavilion + aurora node + process exporter** — module imported on both hosts (`machines/{pavilion,aurora}/default.nix`), scrape jobs in pi VM (`modules/services/victoriametrics.nix`). `just remote pavilion rebuild` / `just remote aurora rebuild` blocked on SSH host-key drift after laptop reinstalls — `ssh-keygen -R <tailnet-hostname>` + `ssh-keyscan -H` against `<host>.saola-matrix.ts.net` (NOT the IP — `just remote` uses tailnet hostnames). Once deployed, verify scrape health at `curl http://100.100.71.3:8428/api/v1/targets?state=active`.

- **MemoryHigh caps on workstation heavy services** — process-exporter is now publishing `namedprocess_namegroup_memory_bytes{memtype="resident",groupname=…}` to pi VM. Wait ≥7 days of data, identify the slowest-growing services, cap each via `systemd.services.<n>.serviceConfig.MemoryHigh = "…G"`. Premature to cap blindly. Sample query: `topk(10, max_over_time(namedprocess_namegroup_memory_bytes{memtype="resident"}[7d]) - min_over_time(namedprocess_namegroup_memory_bytes{memtype="resident"}[7d])) / 1024 / 1024`.
