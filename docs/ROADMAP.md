...
- **Aurora migration — workstation-as-compute / aurora-as-family-vault.** Reorganises hosts to let workstation sleep when no GPU/transcode/bulk-storage workload is active and to give irreplaceable media a 3-copy replication posture. Full delta table, phase ordering, validation gates, and reversibility ladder in `docs/superpowers/plans/2026-06-11-aurora-migration.md`. **Progress (2026-06-12):** P1–P14 ✓ landed (foundation + aurora bootstrap + data move + service-state migration + the entry-plane flip itself). Family-tier surface now answers from pi; workstation closure dropped ~96 MB; 22 routes verified end-to-end from every tailnet host. **Outstanding:** P15 (replication — sender + receiver modules drafted `04629a6` + `ba38187`, awaits operator ssh-key bootstrap), P18 (hypridle suspend re-enabled; blocked on s2idle resume hang diagnose), P19 (WoL — NIC config landed `54a39d8`, magic-packet test gated on P18), P20 (sleep policy + power baseline — gated on P18+P19). P16 pavilion tertiary replica future-work; P17 Hetzner explicitly rejected per ADR-0002.
...
- **Cross-host maintenance coordination for Gatus (G3).** When a host rebuilds, *its own* Gatus is masked by `just rebuild` (G1, landed 2026-06-15) — local probes don't alert on locally-restarting services. But *another* host's Gatus probing the rebuilding one still fires (e.g. workstation's Gatus probing pi while pi rebuilds — once workstation grows its own instance). The correct shape: a per-host maintenance flag readable across hosts (HTTP endpoint or shared file) that every Gatus consults before alerting. Either a Gatus extension/fork that supports flag-based silencing, or a thin sidecar that mutates Gatus's alerting provider config at rebuild start/stop. **Trigger to revisit:** when a second Gatus instance lands on the homelab, or when cross-host probe flapping during rebuilds becomes annoying enough to outweigh the build cost.
