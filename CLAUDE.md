...
- **Internet-public exposure (Tailscale Funnel)**: `nori.funnelRoutes.<n>` is the effect for self-deployed apps that need public-internet reach (operator's portfolio projects). Each entry contributes a path-mounted handler; the composer assembles ONE serve config and applies via imperative `tailscale serve` + `tailscale funnel` commands (the JSON `set-config` schema has been a moving target — imperative + tailscaled-side state persistence is rock-solid). Funnel runs on **port 8443** internally because **Caddy owns 443** for `*.nori.lan`; Tailscale's public-edge maps internet :443 → local :8443 transparently, so visitor URLs stay clean (`https://workstation.<tailnet>.ts.net/<n>/`, no port shown). Distinct effect from `nori.lanRoutes` (tailnet-public, audience-gated) because the threat model is materially different — Funnel exposes to anyone on the internet, no tailnet-as-auth. Schema in `modules/effects/funnel-route.nix`.
- **Self-deployed apps + app secrets**: `secrets/apps.yaml` (separate sops file from `secrets/secrets.yaml`) holds tokens for operator's personal apps deployed on the homelab; `.sops.yaml`'s `secrets/.*\.yaml$` regex covers both with the same recipient set. Per-secret `sopsFile = ../../secrets/apps.yaml` override on the consuming module side. Naming convention: agnostic (`tmdb-token` not `filmder-tmdb-token`) when multiple projects could plausibly share the same key. Live shape worked example: `modules/server/filmder.nix` — sops decrypt → systemd build oneshot (manual trigger via `just deploy-app filmder`, sentinel-skip on idempotent rebuilds) → atomic publish to `/var/lib/<n>/dist` → Tailscale Funnel.
...
- **npm-on-systemd needs bash on PATH gotcha → gotchas.md** — surfaced during filmder Phase A1: npm postinstall scripts (notably `@swc/core`) spawn `sh` for platform detection, but systemd units inherit a minimal env without `/bin/sh`. Symptom: `npm error enoent spawn sh ENOENT`. Fix is `path = with pkgs; [ bash git nodejs_22 ];` on any Node-build systemd unit. Will recur for heim/drinks/finnbydel — worth a gotchas.md entry so the second agent doesn't re-debug.
- **Phase B: heim, drinks, finnbydel deploys** — filmder shipped tonight as the worked example for internet-public deploy. Remaining 3 use the same skeleton (`apps.yaml` secret + `<app>-build.service` + `nori.funnelRoutes.<n>`), with project-specific variations. **heim** (Turborepo + Next.js 15-canary + Payload + Postgres): needs Phase A2 first (shared `modules/server/postgres.nix` with per-app DB + role + sops password — heim and finnbydel both want a real postgres). **drinks** (Apollo GraphQL + npm + Postgres): same shape; runtime systemd unit instead of static dist (`tailscale serve --set-path /drinks/ http://127.0.0.1:<port>`). **finnbydel** (T3 stack: Next.js 13 + tRPC + Prisma): per `.env.example` defaults to sqlite — operator referenced postgres but the code is sqlite; clarify before deploying. Plan was: Phase A2 land postgres, then dispatch 3 parallel agents (one per project) to produce structured deployment-requirement reports, main agent synthesises modules in one coordinated pass.
...
| **6** | `nori.<X>` Reader+Writer effect family (incl. `funnelRoutes` for internet-public exposure, distinct from tailnet-public `lanRoutes`); topology registry; `just ports` live oracle; `nori.fs` as named-context for paths | CLAUDE.md routing table; `docs/PROCEDURES.md`; skills as on-demand context; `git log` narrative; `nix eval .#lib.fragmentNames` lists available dev fragments | skill auto-discovery via descriptions; memory files in `~/.claude/projects/...`; system reminders surface deferred tools |
