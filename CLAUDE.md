...
- **Extend SSO to remaining services** — pending validation of the *arr-stack forward-auth + Jellyseerr-OIDC pattern. Candidates by mode:
  - **Native OIDC** (preferred where supported, preserves per-user identity inside the app): Immich (`photos`) — biggest UX win, family-facing photo timeline. Komga (`comics`) — Spring Security OAuth2 in 1.x. Beszel (`metrics`) — already tracked separately above as "Beszel SSO consumer config".
  - **Forward-auth** (uniform Caddy gate where native OIDC isn't clean): calibre-web (`books`) — exempt `/opds/*` so e-reader apps keep working. Possibly ntfy (`alert`) if push API can be cleanly exempted.
  - **Skip / problematic**: Jellyfin (`media`) — mobile/TV clients use direct API tokens that bypass cookie-based forward-auth; native SSO is plugin-based with sharp historical edges, defer until plugin OIDC stabilizes. Radicale (`calendar`) — CalDAV clients can't follow forward-auth redirects, must stay on htpasswd. Glance/Gatus (`home`/`status`) — intentionally public dashboards. Syncthing (`sync`) — single-admin, low value.
  Pursue once the *arr migration's been stable for ~a week; the marginal Authelia uptime risk per added forward-auth route is real.
