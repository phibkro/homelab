...
- **Phase B: heim, drinks, finnbydel deploys** — filmder shipped on tailnet as the worked example (`https://filmder.nori.lan`). Remaining 3 use the same skeleton (`apps.yaml` secret + `<app>-build.service` + `<app>-serve.service` + `nori.lanRoutes.<n>`), with project-specific variations. **heim** (Turborepo + Next.js 15-canary + Payload + Postgres): needs Phase A2 first (shared `modules/server/postgres.nix` with per-app DB + role + sops password — heim and finnbydel both want a real postgres). **drinks** (Apollo GraphQL + npm + Postgres): same shape; runtime systemd unit (Node serve) instead of static darkhttpd. **finnbydel** (T3 stack: Next.js 13 + tRPC + Prisma): per `.env.example` defaults to sqlite — operator referenced postgres but the code is sqlite; clarify before deploying. Plan: Phase A2 land postgres, then dispatch 3 parallel agents (one per project) to produce structured deployment-requirement reports, main agent synthesises modules in one coordinated pass.
