...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    with subtest("daemon carries VMM isolation policy"):
        props = workstation.succeed("systemctl show agent-engine-self-hosted-launcher.service -p ProtectHome -p ProtectSystem -p RestrictAddressFamilies -p MemoryMax -p TasksMax -p Slice -p DelegateSubgroup -p KillMode")
        ...
        assert "Slice=adlc-firecracker.slice" in props and "AF_NETLINK" in props and "DelegateSubgroup=launcher" in props and "KillMode=control-group" in props, props
  '';
}
