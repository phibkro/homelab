/**
  E2E phases 1-7 — reusable entry-plane NixOS module contract.

  This fixture is not a Pi deployment or host configuration. Production Pi is managed
  by Ansible under `pi/`. This QEMU-only NixOS fixture verifies that reusable
  service modules still compose and reach active state. Per-phase scope:
  ...
  `just e2e-shell e2e-entry-plane-modules` to open the interactive driver +
  iterate on testScript fragments without rebuilding.

  Invoked via nix build .#checks.<system>.e2e-entry-plane-modules.
*/
{
  pkgs,
  lib,
  ...
  nodes.pi =
    { config, lib, ... }:
    {
      imports = [
        ...
        ../infra/common/nixos/storage # nori.fs (consumed by backup/btrbk)
        ...
        # Phase 6 — authelia OIDC. Imports the real runtime path
      ...
      # Production receives this endpoint through the inventory compiler.
      # This standalone module test projects the same manifest data directly
      # so Caddy exercises Authelia's public route without duplicating it.
      nori.lanRoutes.auth = (import ../services/authelia/manifest.nix).endpoints.auth;
      ...
      # pattern in services/restore-drill/nixos.nix etc — %n would
      ...
      # at eval. services/restic-backup/nixos.nix declares it but only
