...
source "$repo_root/infra/pi/scripts/vm-lifecycle.sh"
...
if [[ "$reuse_vm" == true ]]; then
  ...
  sed "s|__SSH_PUBLIC_KEY__|$(<"$ssh_key.pub")|" \
    infra/pi/tests/vm/user-data.yml > "$state_dir/user-data.yml"
  cloud-localds \
    --network-config=infra/pi/tests/vm/network-config.yml \
    "$seed" \
    "$state_dir/user-data.yml" \
    infra/pi/tests/vm/meta-data.yml
fi
...
vm_run_logged enabled-first.log ansible-playbook -i infra/pi/inventory/test.yml -e "@$backup_test_vars" infra/pi/playbooks/pi.yml
vm_phase enabled-second-convergence
vm_run_logged enabled-second.log ansible-playbook -i infra/pi/inventory/test.yml -e "@$backup_test_vars" infra/pi/playbooks/pi.yml
...
readonly disable_playbook="infra/pi/tests/vm/disable-backups.yml"
vm_phase disabled-first-convergence
vm_run_logged disabled-first.log ansible-playbook -i infra/pi/inventory/test.yml -e '{"pi_backup_enabled":false}' "$disable_playbook"
vm_phase disabled-second-convergence
vm_run_logged disabled-second.log ansible-playbook -i infra/pi/inventory/test.yml -e '{"pi_backup_enabled":false}' "$disable_playbook"
