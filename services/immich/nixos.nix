...
in
{
  /*
    ...
    family-vault photos subvolume, including scheduled database dumps
    in its backups/ directory. The live database and ML weights stay
    on root NVMe. Existing user-organized photos sit alongside the
  ...
  */
  ...
  nori.backups.immich.skip = "Photos and scheduled DB dumps at ${config.services.immich.mediaLocation}/backups are covered by media-irreplaceable (@photos).";
}
