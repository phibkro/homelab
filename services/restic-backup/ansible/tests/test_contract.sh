...
rg -q 'pi_backup_enabled: false' "$role_dir/defaults/main.yml" || fail "backup activation must be explicit"
...
rg -q 'HostName=\{\{ pi_backup_target_address \}\}' "$role_dir/templates/backup.sh.j2" || fail "backup target tailnet address override missing"
rg -q 'HostKeyAlias=\{\{ pi_backup_target_host \}\}' "$role_dir/templates/backup.sh.j2" || fail "backup target host-key alias missing"
...
rg -q 'pi_backup_target_host: ""' "$role_dir/defaults/main.yml" || fail "destination must be explicit"
