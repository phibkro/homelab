...
set -euo pipefail
mode="${1:?usage: check-nix.sh fast|all [--impure] | vm [VM_NAME] [--impure]}"
shift
case "$mode" in fast|vm|all) ;; *) echo "Unknown check group: $mode" >&2; exit 2;; esac
name=""
...
')"
if [[ "$mode" == all ]]; then
  selected="$(jq -er 'keys | if length == 0 then error("Empty check set") else .[] end' <<<"$checks")"
else
...
build_args=()
# Modes containing VMs admit only one heavy Nix job at a time.
if [[ "$mode" == vm || "$mode" == all ]]; then build_args=(--max-jobs 1); fi
