...
2. `inventory/` and `modules/infra/` — declared identities plus the `nori.<X>` effect interfaces
3. `modules/machines/base/` — common host realization
4. Five to seven representative `modules/services/` entries — sample shapes, do not read every service
5. `modules/machines/<host>/` — per-host realization
6. `Justfile` — operator workflows
7. `docs/` — rationale; trust code when they conflict
8. `git log --oneline -20` — recent narrative
9. `CLAUDE.md` last — compare its leverage map with the independent read

Treat the leverage map as dated evidence, not authority. Confirm most placements,
refine drifted ones, and add gaps before synthesizing.
