...
2. Run `scripts/juci_procedure.py review <procedure>` and show the normalized
   summary, SHA-256, calls, rollback, and required grants to the operator.
...
```bash
python3 scripts/juci_procedure.py review procedure.json
```
...
```bash
python3 scripts/juci_procedure.py apply procedure.json \
