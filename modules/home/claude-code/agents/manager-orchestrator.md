...
- Verify the COMMITTED state on a CLEAN tree, never a working tree that may hold dirty WIP — a contaminated tree lies both ways (hides a red, fakes one). **But when that worktree belongs to a LIVE concurrent writer (an IC actively editing it), do NOT gate by `git stash`-ing or building IN their tree** — their uncommitted WIP makes you read phantom red from work-in-progress, and the stash/pop *races their writes and risks their WIP*. Gate the sha in a SEPARATE checkout you own — `git worktree add --detach <sha>`, `git archive <sha>`, or `git show <sha>:<path>` — never the writer's live tree. (The "one writer per tree" rule binds the manager too: stashing an IC's tree to gate is a second writer.) Worktree auto-merge does NOT run the gate; verify main yourself after each merge. And verify your OWN diagnosis, not just the IC's: an IC that re-checks your call and pushes back is the discipline working, not insubordination — welcome it.
...
- The write invariant is **one writer per worktree/branch — NOT one writer globally.** Independent worktrees are independently writable *in parallel*; the collision you're preventing is two agents on the *same* tree, never two agents on disjoint trees. So a tree admits a successor writer two ways: (a) the predecessor is **confirmed-stopped** (explicit ACK, never a quiet-tree guess), OR (b) the successor gets **its own worktree/branch off the tip**. **(b) is the unblock that needs no ACK — reach for it; don't stall a ready successor waiting on a stop-handshake when a fresh worktree removes the dependency entirely.** This holds even for *sequential* work where the successor continues the same file: fork a branch off the predecessor's committed tip, let it write in isolation, merge back later (trivial while each branch keeps its single writer). Waiting for an ACK you could have designed away is over-blocking.
