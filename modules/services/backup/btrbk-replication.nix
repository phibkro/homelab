...
let
  ...
  # direction, different host). The receive user runs btrbk via sudo
  # so it can `btrfs subvolume create / receive`; the sudoers
  # provision and the user's authorized_keys live on the workstation
  # side (see TODO at the bottom of this module).
  sshTarget = "btrbk@${config.nori.hosts.workstation.tailnetIp}";
...
lib.mkMerge [
  ...
  (lib.mkIf
    (config.networking.hostName == "aurora" && config.nori.services.btrbk-replication.enabled)
    {
  # ── btrbk instance ─────────────────────────────────────────────
  ...
  systemd.services.btrbk-family-replica.unitConfig.OnFailure = [ "notify@btrbk-family-replica.service" ];
