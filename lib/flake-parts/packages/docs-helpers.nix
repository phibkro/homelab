      ...
      packages =
        let
          ...
          */
          ...
          # (consumed by lib/flake-parts/packages/docs-{backups,fs,replicas}.nix).
          # The inline docs-{lan-route,topology,capabilities} below still
          # define their own stripStorePrefix copies for now; they'll move
          # to lib/flake-parts/packages/ in a subsequent phase.
        in
        {
          # docs-backups, docs-fs, docs-replicas extracted to
          # lib/flake-parts/packages/docs-{backups,fs,replicas}.nix
          ...
          docs-lan-route =
            let
              ...
              /*
                ...
                literal repo-relative path (e.g. `infra/common/nixos/routes.nix`)
          ...
          /*
            ...
            Two-section artifact:
              ...
              §1  Hosts at a glance — walks the public inventory values,
                  including each host's deployment owner, and emits the
                  per-host overview table that topology.md used to carry as
            ...
            §1 is built from public inventory VALUES; §2 is built from the
            NixOS OPTIONS tree. nixosOptionsDoc handles
          ...
          */
          docs-topology =
            let
              ...
              # /** */ block from each infra/<host>/hardware.nix.
              ...
              # need.
              hostHardwareDoc = name: mkFileDocstring (../../.. + "/infra/${name}/hardware.nix");
              hardwareSection = pkgs.runCommandLocal "hardware-section" { } (
                lib.concatStringsSep "\n" (
                  ...
                  ++ map (n: "cat ${hostHardwareDoc n} >> $out") nixosHostNames
                )
              );
            in
          ...
          /*
            ...
            Two-section artifact:
              ...
              §1  Capabilities concern overview — file-level docstring
                  from infra/common/nixos/service-hardening.nix (nori.harden
                  + FS-namespace adapter narrative).

              §2  GPU access pattern — file-level docstring from
                  infra/common/nixos/gpu.nix (the live driver
