...

{
  /*
    ...
    from nori.fs entries via src/infra/common/nixos/storage/default.nix:
      ...
      share → /srv/share   (emitted by nori.fs.share.samba — declared
                             alongside disko at src/infra/workstation/disko.nix)
  ...
  */
    ...
    settings = lib.mkMerge [
      {
        global = {
          ...
          /*
            ...
            library (firewall opens :445 only on waydroid0, below).
          */
        ...
        /*
          ...
          src/infra/common/nixos/storage/default.nix from the per-fs `samba = { … }`
      ...
      /*
        ...
        on the downloads declaration so a host without that dataset
        does not expose this workstation-specific media root.
      */
  ...
  /*
    ...
    tmpfiles via src/infra/common/nixos/storage/default.nix; the four below cover the
