...
in
{
  /**
    ...
    State at /var/lib/private/victorialogs (DynamicUser, upstream module).
    The production equivalent is rendered by `src/services/victorialogs/ansible` and is
    treated as ephemeral: Pi's flash storage is anti-write, log data is
    event-history, and the alert path (ntfy + Gatus) is independent of this
    index. Revisit if/when Pi gains a real disk.
  */
