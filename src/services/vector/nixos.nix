...
in
{
  /**
    ...
    upload (the previous incumbent at nix/modules/system/base/journal-upload.nix, path-coherence: skip — historical
    ...
    settings = {
      ...
      */
      transforms.relabel = {
        ...
        inputs = [ "journald" ];
        source = ''
          # Trusted systemd fields are prefixed with `_` in Vector's # multi-line: ok
          # journald output (user-set fields are bare). Promote a few
          # to friendly top-level names so query syntax stays short.
          .unit = ._SYSTEMD_UNIT
          .pid = ._PID
          .priority = .PRIORITY
          # `.timestamp` (which VictoriaLogs reads via _time_field) is # multi-line: ok
          ...
          # src/services/victorialogs/ansible/defaults/main.yml) get silently
          ...
          # Promote a small, conventional set. Adding more here grows # multi-line: ok
