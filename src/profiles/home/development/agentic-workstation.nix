...
let
  ...
  soul = builtins.readFile ../../../users/nori/programs/agent-soul/SOUL.md;
...
*/
{
  ...
  home.packages = [
    ...
    agentPackages.pi
    agentPackages.rtk
    agentPackages.t3code
    agentPackages.t3code-desktop
    codex-notify
    ...
    /*
      ...
      caller (src/services/agent-fix/nixos.nix) now runs `pagu box`, and the
  ...
  /*
    Prefix the provider-neutral SOUL, then append Codex's operator charter.
    Repository semantics and live state belong in repository AGENTS.md files
    and durable handoffs, not in this global context.
  */
