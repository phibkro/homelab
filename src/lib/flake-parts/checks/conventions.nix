        ...
        in
        {
            ...
            pkgs.runCommandLocal "every-service-has-backup-intent"
              ...
              ''
                ...
                if [ "$fail" -eq 0 ]; then
                  ...
                  echo "See src/infra/common/nixos/backup.nix for the schema."
                  exit 1
                fi
              '';
            ...
            pkgs.runCommandLocal "every-service-has-fs-hardening"
              ...
              ''
                ...
                if [ "$fail" -eq 0 ]; then
                  ...
                  echo "See src/infra/common/nixos/service-hardening.nix for the schema."
                  exit 1
                fi
              '';
          ...
          */
          infra-concerns-have-tests =
            let
              expectedRecipes = {
                "src/infra/common/nixos/alerts.nix" = "test-observability";
                "src/infra/common/nixos/backup.nix" = "test-backups";
                "src/infra/common/nixos/service-hardening.nix" = "test-harden";
                "src/infra/common/nixos/storage/default.nix" = "test-fs";
              };
              evaluationOnlySchemas = {
                ...
                "src/infra/common/nixos/wifi.nix" =
            ...
            pkgs.runCommandLocal "infra-concerns-have-tests"
              ...
              ''
                ...
                check_discovered_schemas() {
                  ...
                  done < <(find "$source_root/src/infra/common/nixos" -type f -name '*.nix' | sort)
                ...
                mkdir -p "$negative_root/src/infra/common/nixos"
                cat > "$negative_root/src/infra/common/nixos/unregistered.nix" <<'EOF' # path-coherence: skip — synthetic file is created during the check
                { lib, ... }:
                ...
                if check_discovered_schemas "$negative_root" >"$negative_output" 2>&1; then
                  ...
                  fail=1
                elif ! grep -Fq 'src/infra/common/nixos/unregistered.nix declares options.nori.* but is not accounted for' "$negative_output"; then # path-coherence: skip — expected synthetic diagnostic
