/*
  ...
  `src/lib/machines.nix`. This flake-part imports the factory
  and surfaces its `nixosConfigurations` at the flake level.
  See `src/lib/machines.nix` for the schema, registry, and
  rationale.
*/
{ inputs, ... }:
let
...
in
{
  flake = {
    ...
    lib.noriRecoveryEvidence = machines.inventory.internal.recoveryEvidence;
    lib.noriPiInventory = machines.inventory.internal.piProjection;
