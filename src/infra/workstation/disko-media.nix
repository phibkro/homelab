{ config, ... }:
{
  /*
    ...
    generators in src/infra/common/nixos/backup.nix filter by `tier`. Single
    source of truth for both the wire-format (disko) and the
    service-facing interface — change one, the other is right here
    next to it. See src/infra/common/nixos/storage/default.nix for the schema.
  */
  nori.fs = {
    downloads = {
              ...
              subvolumes = {
                ...
                "@snapshots" = {
                  /*
                    ...
                    root instance also receives its snapshots
                    into workstation-root/ here
