...
/**
  ...
  baseUrl is per-channel, not a single hardcode: src/infra/operator alerts
  ...
  (auth-default-access = deny, src/services/ntfy/nixos/server.nix) — authTokenSecret is the
    ...
    options = {
      ...
      authTokenSecret = lib.mkOption {
        ...
        description = ''
          ...
          running `auth-default-access = deny` (see src/services/ntfy/nixos/server.nix)
