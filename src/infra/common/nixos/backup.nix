...
in
{
  ...
  */
  ...
  options.nori.backupDelivery.enable = mkOption {
    ...
    default = true;
    description = ''
      Realize declared backup intent as scheduled delivery. Inventory-backed
      hosts derive this switch from their canonical backup policy. Disabling
      delivery retains service intent without requiring a destination.
    '';
  };
  ...
  options.nori.backupTargets = mkOption {
    ...
    description = ''
      ...
      `repository = "sftp:restic@backup.example.net:/mnt/backup"`
      yields `sftp:restic@backup.example.net:/mnt/backup/vaultwarden`.
    ...
    example = lib.literalExpression ''
      {
        onetouch = {
          ...
          description = "Verified local OneTouch backup filesystem";
        };
        remote = {
          repository = "sftp:restic@backup.example.net:/mnt/backup";
          description = "Example remote SFTP backup destination";
          extraOptions = [
            "sftp.command='ssh -i /run/secrets/restic-ssh-key restic@backup.example.net -s sftp'"
          ];
        };
        hetzner = {
    ...
    type = types.attrsOf (
      types.submodule {
        options = {
          repository = mkOption {
            type = types.str;
            example = "sftp:restic@backup.example.net:/mnt/backup";
            description = ''
          ...
          extraOptions = mkOption {
            ...
            default = [ ];
            example = [
              "sftp.command='ssh -i /run/secrets/restic-ssh-key -o UserKnownHostsFile=/run/secrets/restic-known-hosts restic@backup.example.net -s sftp'"
            ];
            description = ''
          ...
          tailnetPeer = mkOption {
            ...
            default = null;
            example = "backup.example-tailnet.ts.net";
            description = ''
  ...
  config = mkIf (config.nori.backups != { }) (
    let
      ...
      */
      ...
      activeJobs = lib.optionalAttrs config.nori.backupDelivery.enable (
    ...
    in
    {
      assertions = [
        ...
        }
        {
          ...
          message = ''
            ...
            `systemModule` declaration in src/inventory/hosts.nix). Local
        ...
        }
        {
          ...
          */
          ...
          message = ''
            ...
            src/services/restic-backup/nixos.nix (or override per-host).
          '';
        }
      ];
      ...
      /*
        ...
        src/services/restic-backup/nixos.nix; without it a silent backup
