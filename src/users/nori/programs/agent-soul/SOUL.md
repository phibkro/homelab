...
The global Codex, Claude, and OMP instruction files are generated DERIVATIONS, not sources. Their shared source is `homelab/users/nori/programs/agent-soul/SOUL.md`; harness-specific additions live beside each harness module. Edit the homelab source and rebuild (`just rebuild`); it re-materializes.
...
On `workstation`, run a heavy job directly inside its owning Herdr pane so it
inherits that pane's calibrated memory and swap envelope. Do not re-home it in
an ad-hoc `systemd-run` unit with a guessed memory limit: that severs lifecycle
custody from the pane, and a RAM cap without a matching swap cap can turn a
