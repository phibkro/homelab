...
in
{
  /*
    ...
    The Paperless sink (src/services/paperless/nixos.nix) owns everything
  ...
  options.nori.papersFetch = {
    consumeDir = lib.mkOption {
      ...
      description = ''
        ...
        invoking operator must be able to write it. Check the local
        services.paperless.consumptionDirIsPublic setting and directory permissions.
      '';
    };
