...
let
  ...
  */
  ...
  /*
    ...
    lives in its own subdirectory (src/users/nori/programs/desktop/hypr-rice/
  ...
  */
  ...
  checkedHyprlandLua = pkgs.runCommandLocal "hyprland.lua" { } ''
    ...
    ${pkgs.gnugrep}/bin/grep -Fq ${lib.escapeShellArg ''dofile("${toString riceAdapter}")''} "$out"
    ${pkgs.gnugrep}/bin/grep -Fq ${lib.escapeShellArg ''"${toString layoutCore}"''} "$out"
...
in
{
  config = lib.mkIf config.nori.hyprRice.enable {
    ...
    # src/users/nori/programs/desktop/hypr-lock.nix already owns hyprlock.settings.background
